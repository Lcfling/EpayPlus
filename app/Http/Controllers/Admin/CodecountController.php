...
class CodecountController extends Controller
{
    ...
    public function index(Request $request){
        ...
        if(true==$request->has('user_id')){
            $codecount->where('users_count.user_id','=',$request->input('user_id'));
        }
        if(true==$request->has('creatime')){
            $creatime=$request->input('creatime');
            $start=strtotime($creatime);
            $end=strtotime('+1day',$start);
            $codecount->whereBetween('users_count.creatime',[$start,$end]);
        }
        if(true==$request->has('savetime')){
            $creatime=$request->input('savetime');
            $start=strtotime($creatime);
            $end=strtotime('+1day',$start);
            $codecount->whereBetween('users_count.savetime',[$start,$end]);
        }if(true==$request->input('excel')&& true==$request->has('excel')){
            $head = array('码商ID','上级ID','码商手机号','码商姓名','余额','总分','总佣金','冻结金额','提现总金额','提现实际到账总金额','总充值','总上分','总下分','创建时间');
            $excel = $codecount->leftJoin('users','users_count.user_id','=','users.user_id')
                               ->select('users.user_id','users.pid','users.account','users.name','users_count.balance','users_count.tol_sore','users_count.tol_brokerage','users_count.freeze_money','users_count.drawMoney','users_count.tradeMoney','users_count.tol_recharge','users_count.shangfen','users_count.xiafen','users_count.creatime')
                               ->orderBy('users.user_id','asc')->get()->toArray();
            ...
            $data = $codecount->leftJoin('users','users_count.user_id','=','users.user_id')
                              ->select('users_count.*','users.account','users.name')
                              ->orderBy('creatime','desc')->paginate(10)->appends($request->all());
